...
function trimTitle(data) {
  if (data.title !== undefined) {
    return data.title.length > 20 ? data.title.substring(0, 20) + "..." : data.title;
  }
...
function getShow() {
  ...
  $.ajax({
    ...
    success: function (data) {
      const releaseDate = data.release_date && data.release_date.length > 0
        ? data.release_date
        : data.first_air_date
      ...
      $("#main-title").html(
        data.title && data.title.length > 0 ? data.title + " (" + getYear(releaseDate) + ")" : data.name + " (" + getYear(releaseDate) + ")"
      );
      $("#poster").attr(
        "src",
...
function getRecommendations(data) {
  ...
  data.results.forEach((item) => {
    if (q < 10) {
      ...
      html += `<span class="inline-block px-2 py-1 leading-none bg-orange-200 text-orange-800 rounded-full font-semibold uppercase tracking-wide text-xs">Highlight 2022</span><h2 class="mt-2 mb-2 font-bold">${trimTitle(item)}</h2>`
      html += `</div></a></div>`
    }
  });
...
function setupSeason(data) {
  ...
  seasons.forEach((season) => {
    if (season.name != "Specials") {
      html += `<option class="option ${season.name == "Season 1" ? "active" : ""} text-grey-900" value="${season.name}">${season.name}</option>`
    }
  });
  ...
  for (var i = 0; i < season.episode_count; i++) {
    html += `<option class="option ${i == 0 ? "active" : ""} text-grey-900" value="${i + 1}">${i + 1}</option>`
  }
...
function changeStream() {
  ...
  $("#player").attr("src", "https://www.2embed.ru/embed/tmdb/tv?id=" + id + "&s=" + season + "&e=" + stream);
}
